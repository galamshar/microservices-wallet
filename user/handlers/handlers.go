...
import (
	...
	internalJWT "github.com/galamshar/microservices-wallet/user/internal/jwt"
)
...
func (s *UserhandlerService) DepositToBalance(c *fiber.Ctx) error {
	...
	if success, err := s.StorageService.UpdateBalance(body.FromID, body.Amount); err != nil || success != true {
		return c.Status(fiber.ErrConflict.Code).JSON(fiber.Map{"status": "error", "message": "Error in DB", "data": err.Error()})
	}
