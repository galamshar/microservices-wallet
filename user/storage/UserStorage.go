...
import (
	...
	"github.com/galamshar/microservices-wallet/user/internal/utils"
...
func (s *UserStorageService) UpdateBalance(id string, amount float64) (bool, error) {
	if amount <= 0 {
		return false, errors.New("Incorrect deposit amount!")
	}
	...
	var balance = userDB.Balance

	//Change data in DB

	UserChange := &models.User{Balance: balance + amount, UpdatedAt: time.Now()}
	...
	go func() {
		//Movement of change of Username
		var change string = "Modify Balance from " + fmt.Sprint(balance) + " to " + fmt.Sprint(balance+amount) + id
