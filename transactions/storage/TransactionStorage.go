...
import (
	...
	grpcClient "github.com/galamshar/microservices-wallet/transactions/grpc/client"
	"github.com/galamshar/microservices-wallet/transactions/models"
)
...
func (s *TransactionStorageService) CreateTransaction(transaction models.TransactionWebRequest) (*models.TransactionWebResponse, bool, error) {
	...
	go func() {
		movement := fmt.Sprintf("Transaction of %.2f from %s to %s", transaction.Amount, transaction.FromName, transaction.ToName)
